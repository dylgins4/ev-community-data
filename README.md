# Smart EV Guides — Community Vehicle Data

Help us add more electric vehicles to the [Smart EV Guides](https://apps.apple.com/us/app/smart-ev-guides/id6755702439) app — used by first responders to quickly look up EV rescue procedures during emergencies.

**We need rescue sheet PDFs for EVs not yet in the app. Want to add your vehicle???** 

---

## How to Add a Vehicle

1. **Find a rescue sheet PDF** for an EV not in our list below (see [Where to Find Them](#where-to-find-rescue-sheets))
2. **Upload it** to the [`submissions/`](submissions/) folder by [clicking here to upload](../../upload/main/submissions) — or open an [Issue](../../issues/new) and attach the PDF there
3. **Tell us** the make, model, and where you found it
4. **We handle the rest** — we'll review it, process it, and push it to the app

Once approved, the vehicle will appear in the app for all users automatically.

---

## Where to Find Rescue Sheets

Rescue sheets are official manufacturer documents that show first responders how to safely handle an EV. Here are some places to find them:

- [Euro NCAP Rescue Sheets](https://www.euroncap.com/en/vehicle-safety/rescue-sheets/) — large free database
- [NFPA Emergency Field Guide](https://www.nfpa.org/training-and-events/by-topic/alternative-fuel-vehicle-safety-training/emergency-field-guide) — US-focused
- Many manufacturers publish them on their websites — try googling **"[car name] rescue sheet PDF"**

We're looking for PDFs that include things like:
- Where the high-voltage battery and cables are
- Safe cut zones for extrication
- How to disable the high-voltage system
- Fire and submersion response steps

---

## What's Already in the App (78 vehicles)

<details>
<summary>Click to see the full list</summary>

| Make | Models |
|------|--------|
| Acura | ZDX |
| Audi | e-tron, e-tron GT, Q4 e-tron, Q6 e-tron, Q8 e-tron |
| BMW | i4, i5, i7, iX |
| Cadillac | Celestiq, Escalade IQ, Lyriq, Optiq |
| Chevrolet | Blazer EV, Bolt, Equinox EV, Silverado EV |
| Dodge | Charger Daytona 2dr, Charger Daytona 4dr |
| Ford | E-Transit, F-150 Lightning, Mustang Mach-E |
| Genesis | G80 EV, GV60, GV70 EV |
| GMC | Hummer EV Pickup, Hummer EV SUV, Sierra EV |
| Honda | Prologue |
| Hyundai | Ioniq 5, Ioniq 5 N, Ioniq 6, Ioniq 9, Ioniq Electric, Kona Electric |
| Jeep | Wagoneer S |
| Kia | EV6, EV9, Niro EV |
| Lexus | RZ 450e |
| Lucid | Air, Gravity |
| Mazda | MX-30 |
| Mercedes | EQE, EQS SUV, G 580 E |
| Mini | Countryman SE |
| Nissan | Ariya, Leaf |
| Polestar | 2, 3, 4 |
| Porsche | Cayenne E-Hybrid, Macan Electric, Taycan |
| Ram | ProMaster EV |
| Rivian | Delivery Van, R1S, R1T |
| Rolls-Royce | Spectre |
| Subaru | Solterra |
| Tesla | Cybertruck, Model 3, Model S, Model X, Model Y, Roadster, Semi |
| Toyota | bZ, bZ4X, C-HR EV |
| Volkswagen | Golf GTE, ID.4, ID. Buzz |
| Volvo | EC40, EX30, EX40, EX90 |

</details>

If your EV isn't on this list, we'd love your help adding it!

---

## Submission Tips

- **English PDFs only** for now
- **One vehicle per submission**
- **Official documents only** — from the manufacturer or a recognized safety organization
- **Please don't submit documents behind paywalls**

---

## Questions?

Open an [Issue](../../issues/new) — happy to help!

## License

Submitted rescue sheet PDFs remain the intellectual property of their respective manufacturers. This repository organizes publicly available safety documents for use in emergency response applications.



